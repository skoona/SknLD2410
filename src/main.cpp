 ...
 *
 * Where [reporting data], can be CSV-like, JSON with Short or Long attributes
 * as documented in [SerialStudio's
 * page](https://github.com/Serial-Studio/Serial-Studio/wiki/Communication-Protocol)
 *
 * WIFI_SSID and WIFI_PASS are double-quoted environment variables with related
 * values,
 * -- example: export PLATFORMIO_BUILD_FLAGS=-DWIFI_PASS='"ssid-password"'
 * -DWIFI_SSID='"ssid-value"' or
 ...
 * - Factory Settings are fine as is, with the exception of distance gate and idle-delay-time.
 * - So setting 'setmaxvalues 8 8 300' motion stationary idle-delay-time-seconds is useful.
 * - each gate is a number 0 thru 8, and controls the target distance the device being triggered.
 * 
 */
...
uint16_t remotePort =
    8090; // default value, will be overridden on reciept of udp request
IPAddress
    ipRemote(10, 100, 1,
             5); // default value, will be overridden on reciept of udp request
...
void sendToRequestor(String str, bool requestor = false) {
  ...
  // Serial.printf("DEBUG: SizeOf(serialBuffer)=%d Length(str)=%d Contents:%s",
  // sizeof(serialBuffer), str.length(), str.c_str());
  udp.print(str);
  return udp.close();
}
#endif
...
void setup(void) {
  ...
  Serial2.begin(256000, SERIAL_8N1, RXD2,
                TXD2); // UART for monitoring the radar rx, tx
